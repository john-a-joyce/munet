      ...
      list mounts {
        ...
        description
          "Mounts to be made inside the namespace. Currently only supported for
           container and qemu based nodes.";
        ...
        leaf type {
          type string;
          description
            "The type of the mount. Includes:
             `bind`: Mount source into node
              - Note: Qemu bind mount is facilitated by 9p network file system
             `tmpfs`: Mount tmpfs into node
             `usb`: Mount source into node (alternative)
              - Note: Qemu-only. Do not write to the source while munet is running!";
        }
      }
      leaf name {
        type string;
